...
Sophia plays the uploaded jazz dance once after collecting the final gem, facing the camera. The next-level panel appears immediately with the dance in its own canvas; starting the next level restores her running action. `sophia-level-complete.glb` contains only animation and its named hierarchy, reusing the existing Sophia mesh and textures. Rebuild with `python tools/prepare_celebration.py "path/to/sophie level complete animation.glb"`.
...
The victory popup displays Sophia in a dedicated lit 3D canvas, playing the selected dance once and holding the final pose. Next level is available immediately. The existing animated character returns to the game scene on restart.
